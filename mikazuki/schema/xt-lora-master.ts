    ...
    Schema.intersect([
        Schema.intersect([
            Schema.object({
                ...
                pretrained_model_name_or_path: Schema.string().role('filepicker').default("C:/stable-diffusion-webui-1.6.0/models/Stable-diffusion/国风3 GuoFeng3_v3.4.safetensor").description("底模文件路径"),
...
//                     enable_preview: Schema.const(true).required(),
                    sample_prompts: Schema.string().role('textarea').default("(masterpiece, best quality:1.2),  --n lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts,signature, watermark, username, blurry,  --w 512  --h 512  --l 7  --s 24  --d 1337"),
