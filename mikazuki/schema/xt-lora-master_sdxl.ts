Schema.intersect([
    ...
    Schema.object({
        pretrained_model_name_or_path: Schema.string().role('filepicker').default("C:/stable-diffusion-webui-1.6.0/models/Stable-diffusion/官方底模/sd_xl_turbo_1.0_fp16.safetensors").description("底模文件路径"),
        ...
        train_batch_size: Schema.number().min(1).default(6).description("批量大小"),
        gradient_accumulation_steps: Schema.number().min(1).default(2).description("梯度累加步数"),
        sample_prompts: Schema.string().role('textarea').default("(masterpiece, best quality:1.2),  --n lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts,signature, watermark, username, blurry,  --w 1024  --h 1024  --l 7  --s 24  --d 1337").description("预览图提示词"),
    ...
    Schema.intersect([
        ...
        Schema.object({
            output_name: Schema.string().default("at").description("模型保存名称"),
        ...
        Schema.intersect([
            Schema.object({
                learning_rate: Schema.string().default("1").description("总学习率，在分开设置 U-Net 与文本编码器学习率后这个值失效。"),
                unet_lr: Schema.string().default("1").description("U-Net 学习率"),
                text_encoder_lr: Schema.string().default("1").description("文本编码器学习率"),
                ...
                ]).default("cosine").description("学习率调度器设置"),
            ...
            Schema.object({
                optimizer_type: Schema.union([
                    ...
                    "AdaFactor",
                    "Prodigy"
                ]).default("Prodigy").description("优化器设置"),
    ...
    //                 network_module: Schema.const('lycoris.kohya').required(),
        ...
        Schema.intersect([
            Schema.object({
                enable_preview: Schema.boolean().default(true).description('启用训练预览图'),
            }).description('训练预览图设置'),

            Schema.union([
                Schema.object({
//                     enable_preview: Schema.const(true).required(),
                    sample_prompts: Schema.string().role('textarea').default("(masterpiece, best quality:1.2),  --n lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts,signature, watermark, username, blurry,  --w 1024  --h 1024  --l 7  --s 24  --d 1337"),
                    ...
                    sample_every_n_epochs: Schema.number().default(1).description("每 N 个 epoch 生成一次预览图"),
                }),
                Schema.object({}),
            ]),
        ]),
        ...
        Schema.object({
            ...
            full_bf16: Schema.boolean().default(true).description("完全使用 BF16 精度"),
            fp8_base: Schema.boolean().description("对基础模型使用 FP8 精度"),
            no_half_vae: Schema.boolean().default(true).description("不使用半精度 VAE"),
